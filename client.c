   ...
   bzero(&info,sizeof(info)); 
    while(1){
        ...
        else if(!strcmp(tmp,"upload"))
        {
	        ...
	        while(!feof(file))
            {
  	            fread(filedata+num,sizeof(char),1,file);++num;
            }
            for(int i=0;i<num;++i){
                printf("%c",filedata[i]);
            }
            unsigned int size = compress(filedata);
            printf("Original file length: %d bytes, compressed file length: 23,768 bytes (ratio: 16.95%)",num);
        }
        else{}
